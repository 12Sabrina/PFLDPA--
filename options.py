...
def parse_args():
    ...
    parser.add_argument('--user_sample_rate', type=float, default=1, help="Sample rate for user sampling")##可选（随机选择的客户端用于训练的比例）
    ...
    parser.add_argument('--noise_multiplier', type=float, default=0.6)##可选噪声乘子为：{0, 0.01, 0.03, 0.1}
    parser.add_argument('--target_unclipped_quantile', type=float, default=0.5)
    parser.add_argument('--adaptive_clip_learning_rate', type=float, default=0.1)
    parser.add_argument('--model', type=str, default='resnet18_v2')
    parser.add_argument('--fisher_threshold', type=float, default=0.3, help="Fisher information threshold for parameter selection")
    parser.add_argument('--lambda_1', type=float, default=0.05, help="Lambda value for EWC regularization term")
    parser.add_argument('--lambda_2', type=float, default=0.06, help="Lambda value for regularization term to control the update magnitude")
    ...
    parser.add_argument('--dataset', type=str, default='EMNISTc')
